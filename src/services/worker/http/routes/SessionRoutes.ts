...
import { DashScopeAgent, isDashScopeSelected, isDashScopeAvailable } from '../../DashScopeAgent.js';
...
export class SessionRoutes extends BaseRouteHandler {
  ...
  private getActiveAgent(): SDKAgent | GeminiAgent | OpenRouterAgent | DashScopeAgent {
    if (isDashScopeSelected()) {
      if (isDashScopeAvailable()) {
        logger.debug('SESSION', 'Using DashScope agent');
        return this.dashScopeAgent;
      } else {
        throw new Error('DashScope provider selected but no API key configured. Set CLAUDE_MEM_DASHSCOPE_API_KEY in settings or DASHSCOPE_API_KEY environment variable.');
      }
    }
  ...
  private startGeneratorWithProvider(
    ...
    provider: 'claude' | 'gemini' | 'openrouter' | 'dashscope',
    source: string
  ): void {
    if (!session) return;
    ...
    const agentMap = {
      dashscope: { agent: this.dashScopeAgent, name: 'DashScope' },
