...
export function Header({
  ...
  async function handleShutdown() {
    ...
    try {
      ...
      setPowerStatus(t('header.shuttingDown'));
      setTimeout(() => setPowerStatus(null), 3000);
    } catch {
      setPowerStatus(t('header.connectFailed'));
      setTimeout(() => setPowerStatus(null), 3000);
    }
  }
    ...
    {showShutdownConfirm && (
      <div className="power-confirm-backdrop" onClick={() => setShowShutdownConfirm(false)}>
        <div className="power-confirm-dialog" onClick={e => e.stopPropagation()}>
          <div className="power-confirm-title">{t('header.shutdownConfirm')}</div>
          <div className="power-confirm-msg">
            {t('header.shutdownMsg')}
          </div>
          <div className="power-confirm-actions">
            <button className="power-confirm-cancel" onClick={() => setShowShutdownConfirm(false)}>{t('header.cancel')}</button>
            <button
              className="power-confirm-ok power-confirm-danger"
              onClick={handleShutdown}
            >
              {t('header.shutdownBtn')}
            </button>
          </div>
        </div>
      </div>
    )}
    {powerStatus && <div className="power-status-toast">{powerStatus}</div>}
  </>
  );
}
