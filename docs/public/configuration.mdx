...
| `CLAUDE_MEM_GEMINI_API_URL`   | `https://generativelanguage.googleapis.com/v1beta/models` | Custom API URL for Gemini-compatible endpoints |
| `CLAUDE_MEM_GEMINI_MODEL`     | `gemini-2.5-flash-lite`          | Gemini model: `gemini-2.5-flash-lite`, `gemini-2.5-flash`, `gemini-3-flash` |
| `CLAUDE_MEM_GEMINI_RATE_LIMITING_ENABLED` | `true`             | Enable rate limiting for free tier (disable if billing enabled) |
