...
| `CLAUDE_MEM_GEMINI_API_URL` | string | `https://generativelanguage.googleapis.com/v1beta/models` | Custom API URL for Gemini-compatible endpoints |
| `CLAUDE_MEM_GEMINI_MODEL` | `gemini-2.5-flash-lite`, `gemini-2.5-flash`, `gemini-3-flash` | `gemini-2.5-flash-lite` | Gemini model to use |
| `CLAUDE_MEM_GEMINI_RATE_LIMITING_ENABLED` | `true`, `false` | `true` | Enable rate limiting for free tier (disable if billing enabled) |
...
export GEMINI_API_URL="https://your-custom-endpoint.com/v1/models"  # optional
```

The settings file takes precedence over environment variables.

### Custom API Endpoint

For Gemini-compatible services (e.g., self-hosted proxies or alternative endpoints), you can configure a custom API URL:

```json
{
  "CLAUDE_MEM_PROVIDER": "gemini",
  "CLAUDE_MEM_GEMINI_API_KEY": "your-api-key-here",
  "CLAUDE_MEM_GEMINI_API_URL": "https://your-custom-endpoint.com/v1/models",
  "CLAUDE_MEM_GEMINI_MODEL": "your-model-name"
}
```

<Note>
When using a custom API URL, model name validation is skipped to allow non-standard model names supported by your endpoint.
</Note>
