...
| 34 | `CLAUDE_MEM_EMBEDDING_FUNCTION` | 向量搜索 | `dashscope:text-embedding-v3:768` | Embedding 模型。`dashscope:text-embedding-v3:768`=DashScope text-embedding-v3 (768d)；可选 `default`=all-MiniLM-L6-v2 (384d, 英文)、`sentence-transformers/multilingual-MiniLM-L12-v2` (384d, 50+语言)、`shibing624/text2vec-base-chinese` (768d, 中文)。 | `"default"` | 🆕 新增 |
| 35 | `CLAUDE_MEM_BUDGET_ENABLED` | 预算跟踪 | `false` | 启用预算跟踪功能 | `"true"` | 🆕 新增 |
| 36 | `CLAUDE_MEM_BUDGET_PRESET` | 预算跟踪 | `claude-haiku` | 定价预设 ID：`claude-haiku` / `claude-max` / `custom` 等 | `"claude-max"` | 🆕 新增 |
| 37 | `CLAUDE_MEM_BUDGET_DAILY_LIMIT` | 预算跟踪 | `1.00` | 每日限额（token 计费为 USD，消息计费为条数） | `"5.00"` | 🆕 新增 |
| 38 | `CLAUDE_MEM_BUDGET_MONTHLY_LIMIT` | 预算跟踪 | `20.00` | 每月限额 | `"100.00"` | 🆕 新增 |
| 39 | `CLAUDE_MEM_BUDGET_CUSTOM_PRICING` | 预算跟踪 | `""` | 自定义定价 JSON，仅 preset=`custom` 时生效 | `"{\"input\":0.25,\"output\":1.25}"` | 🆕 新增 |
