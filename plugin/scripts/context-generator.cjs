...
`+JSON.stringify(o,null,2):l=" "+this.formatData(o));let p="";if(n){let{sessionId:u,memorySessionId:b,correlationId:S,...g}=n;Object.keys(g).length>0&&(p=` {${Object.entries(g).map(([O,Tt])=>`${O}=${Tt}`).join(", ")}}`)}let T=`[${i}] [${a}] [${d}] ${c}${s}${p}${l}`,m=Rt(T);if(this.logFilePath)try{(0,A.appendFileSync)(this.logFilePath,m+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),l=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",p={...s,location:l};return this.warn(e,`[HAPPY-PATH] ${t}`,p,n),o}},E=new W;var Ct={};function It(){return typeof __dirname<"u"?__dirname:(0,h.dirname)((0,ce.fileURLToPath)(Ct.url))}var At=It(),R=C.get("CLAUDE_MEM_DATA_DIR"),y=process.env.CLAUDE_CONFIG_DIR||(0,h.join)((0,de.homedir)(),".claude"),qt=(0,h.join)(y,"plugins","marketplaces","thedotmack"),Jt=(0,h.join)(R,"archives"),zt=(0,h.join)(R,"logs"),Qt=(0,h.join)(R,"trash"),Zt=(0,h.join)(R,"backups"),es=(0,h.join)(R,"modes"),ts=(0,h.join)(R,"settings.json"),Ee=(0,h.join)(R,"claude-mem.db"),ss=(0,h.join)(R,"vector-db"),rs=(0,h.join)(R,"observer-sessions"),ns=(0,h.join)(y,"settings.json"),os=(0,h.join)(y,"commands"),is=(0,h.join)(y,"CLAUDE.md");function le(r){(0,_e.mkdirSync)(r,{recursive:!0})}function pe(){return(0,h.join)(At,"..")}var me=require("crypto");var yt=3e4;function ue(r,e,t){return(0,me.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function Te(r,e,t){let s=t-yt;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var F=class{db;constructor(e=Ee){e!==":memory:"&&le(R),this.db=new ge.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createAIAnalysisTable(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.createBudgetTables()}initializeSchema(){this.db.run(`
    ...
    `),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(8,new Date().toISOString()),E.debug("DB","Successfully added hierarchical fields to observations table")}makeObservationsTextNullable(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(9))return;let s=this.db.query("PRAGMA table_info(observations)").all().find(n=>n.name==="text");if(!s||s.notnull===0){this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(9,new Date().toISOString());return}E.debug("DB","Making observations.text nullable");try{this.db.run("BEGIN TRANSACTION"),this.db.run("DROP TABLE IF EXISTS observations_new"),this.db.run(`
    ...
    `),this.db.run("CREATE INDEX IF NOT EXISTS idx_pending_messages_session ON pending_messages(session_db_id)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_pending_messages_status ON pending_messages(status)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_pending_messages_claude_session ON pending_messages(content_session_id)"),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(16,new Date().toISOString()),E.debug("DB","pending_messages table created successfully")}renameSessionIdColumns(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(17))return;E.debug("DB","Checking session ID columns for semantic clarity rename");let t=0,s=(n,o,i)=>{let a=this.db.query(`PRAGMA table_info(${n})`).all(),d=a.some(l=>l.name===o);return a.some(l=>l.name===i)?!1:d?(this.db.run(`ALTER TABLE ${n} RENAME COLUMN ${o} TO ${i}`),E.debug("DB",`Renamed ${n}.${o} to ${i}`),!0):(E.warn("DB",`Column ${o} not found in ${n}, skipping rename`),!1)};s("sdk_sessions","claude_session_id","content_session_id")&&t++,s("sdk_sessions","sdk_session_id","memory_session_id")&&t++,s("pending_messages","claude_session_id","content_session_id")&&t++,s("observations","sdk_session_id","memory_session_id")&&t++,s("session_summaries","sdk_session_id","memory_session_id")&&t++,s("user_prompts","claude_session_id","content_session_id")&&t++,this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(17,new Date().toISOString()),t>0?E.debug("DB",`Successfully renamed ${t} session ID columns`):E.debug("DB","No session ID column renames needed (already up to date)")}repairSessionIdColumnRename(){this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(19)||this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(19,new Date().toISOString())}addFailedAtEpochColumn(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(20))return;this.db.query("PRAGMA table_info(pending_messages)").all().some(n=>n.name==="failed_at_epoch")||(this.db.run("ALTER TABLE pending_messages ADD COLUMN failed_at_epoch INTEGER"),E.debug("DB","Added failed_at_epoch column to pending_messages table")),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(20,new Date().toISOString())}createBudgetTables(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(24))return;if(this.db.query("SELECT name FROM sqlite_master WHERE type='table' AND name='budget_state'").all().length>0){this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(24,new Date().toISOString());return}E.debug("DB","Creating budget tracking tables"),this.db.run("BEGIN TRANSACTION");try{this.db.run(`
      ...
      `).run(n,o,Date.now()),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(24,new Date().toISOString()),this.db.run("COMMIT")}catch(s){throw this.db.run("ROLLBACK"),s}E.debug("DB","Successfully created budget tracking tables")}addOnUpdateCascadeToForeignKeys(){if(!this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(21)){E.debug("DB","Adding ON UPDATE CASCADE to FK constraints on observations and session_summaries"),this.db.run("PRAGMA foreign_keys = OFF"),this.db.run("BEGIN TRANSACTION");try{this.db.run("DROP TRIGGER IF EXISTS observations_ai"),this.db.run("DROP TRIGGER IF EXISTS observations_ad"),this.db.run("DROP TRIGGER IF EXISTS observations_au"),this.db.run("DROP TABLE IF EXISTS observations_new"),this.db.run(`
        CREATE TABLE observations_new (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          memory_session_id TEXT NOT NULL,
          project TEXT NOT NULL,
      ...
      `),this.db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='session_summaries_fts'").all().length>0&&this.db.run(`
          ...
          CREATE TRIGGER IF NOT EXISTS session_summaries_au AFTER UPDATE ON session_summaries BEGIN
            ...
            VALUES (new.id, new.request, new.investigated, new.learned, new.completed, new.next_steps, new.notes);
          END;
        `),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(21,new Date().toISOString()),this.db.run("COMMIT"),this.db.run("PRAGMA foreign_keys = ON"),E.debug("DB","Successfully added ON UPDATE CASCADE to FK constraints")}catch(t){throw this.db.run("ROLLBACK"),this.db.run("PRAGMA foreign_keys = ON"),t}}}addObservationContentHashColumn(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(22))return;this.db.query("PRAGMA table_info(observations)").all().some(n=>n.name==="content_hash")||(this.db.run("ALTER TABLE observations ADD COLUMN content_hash TEXT"),this.db.run("UPDATE observations SET content_hash = substr(hex(randomblob(8)), 1, 16) WHERE content_hash IS NULL"),this.db.run("CREATE INDEX IF NOT EXISTS idx_observations_content_hash ON observations(content_hash, created_at_epoch)"),E.debug("DB","Added content_hash column to observations table with backfill and index")),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(22,new Date().toISOString())}addSessionCustomTitleColumn(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(23))return;this.db.query("PRAGMA table_info(sdk_sessions)").all().some(n=>n.name==="custom_title")||(this.db.run("ALTER TABLE sdk_sessions ADD COLUMN custom_title TEXT"),E.debug("DB","Added custom_title column to sdk_sessions table")),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(23,new Date().toISOString())}updateMemorySessionId(e,t){this.db.prepare(`
      UPDATE sdk_sessions
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,memory_session_id:i,type:a,concepts:d,files:c}=t,l=s==="date_asc"?"ASC":"DESC",p=n?`LIMIT ${n}`:"",T=e.map(()=>"?").join(","),m=[...e],u=[];if(i&&(u.push("memory_session_id = ?"),m.push(i)),o&&(u.push("project = ?"),m.push(o)),a)if(Array.isArray(a)){let g=a.map(()=>"?").join(",");u.push(`type IN (${g})`),m.push(...a)}else u.push("type = ?"),m.push(a);if(d){let g=Array.isArray(d)?d:[d],f=g.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");m.push(...g),u.push(`(${f.join(" OR ")})`)}if(c){let g=Array.isArray(c)?c:[c],f=g.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");g.forEach(O=>{m.push(`%${O}%`,`%${O}%`)}),u.push(`(${f.join(" OR ")})`)}let b=u.length>0?`WHERE id IN (${T}) AND ${u.join(" AND ")}`:`WHERE id IN (${T})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${b}
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),c=ue(e,s.title,s.narrative),l=Te(this.db,c,a);if(l)return{id:l.id,createdAtEpoch:l.created_at_epoch};let T=this.db.prepare(`
      INSERT INTO observations
      (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,c,d,a);return{id:Number(T.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),l=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(l.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),c=new Date(d).toISOString();return this.db.transaction(()=>{let p=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let u of s){let b=T.run(e,t,u.type,u.title,u.subtitle,JSON.stringify(u.facts),u.narrative,JSON.stringify(u.concepts),JSON.stringify(u.files_read),JSON.stringify(u.files_modified),o||null,i,c,d);p.push(Number(b.lastInsertRowid))}let m=null;if(n){let b=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,c,d);m=Number(b.lastInsertRowid)}return{observationIds:p,summaryId:m,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,c){let l=c??Date.now(),p=new Date(l).toISOString();return this.db.transaction(()=>{let m=[],u=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of s){let f=u.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),a||null,d,p,l);m.push(Number(f.lastInsertRowid))}let b;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,p,l);b=Number(f.lastInsertRowid)}return this.db.prepare(`
      ...
      `).run(l,o),{observationIds:m,summaryId:b,createdAtEpoch:l}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,memory_session_id:i}=t,a=s==="date_asc"?"ASC":"DESC",d=n?`LIMIT ${n}`:"",c=e.map(()=>"?").join(","),l=[...e],p=[];i&&(p.push("memory_session_id = ?"),l.push(i)),o&&(p.push("project = ?"),l.push(o));let T=p.length>0?`WHERE id IN (${c}) AND ${p.join(" AND ")}`:`WHERE id IN (${c})`;return this.db.prepare(`
      SELECT * FROM session_summaries
      ${T}
      ORDER BY created_at_epoch ${a}
      ${d}
    `).all(...l)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,memory_session_id:i}=t,a=s==="date_asc"?"ASC":"DESC",d=n?`LIMIT ${n}`:"",c=e.map(()=>"?").join(","),l=[...e],p=[];o&&(p.push("s.project = ?"),l.push(o)),i&&(p.push("s.memory_session_id = ?"),l.push(i));let T=p.length>0?`AND ${p.join(" AND ")}`:"";return this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.memory_session_id
      ...
      ${d}
    `).all(...l)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,c;if(e!==null){let S=`
      ...
      `,g=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let f=this.db.prepare(S).all(e,...a,s+1),O=this.db.prepare(g).all(e,...a,n+1);if(f.length===0&&O.length===0)return{observations:[],sessions:[],prompts:[]};d=f.length>0?f[f.length-1].created_at_epoch:t,c=O.length>0?O[O.length-1].created_at_epoch:t}catch(f){return E.error("DB","Error getting boundary observations",void 0,{error:f,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let S=`
      ...
      `,g=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let f=this.db.prepare(S).all(t,...a,s),O=this.db.prepare(g).all(t,...a,n+1);if(f.length===0&&O.length===0)return{observations:[],sessions:[],prompts:[]};d=f.length>0?f[f.length-1].created_at_epoch:t,c=O.length>0?O[O.length-1].created_at_epoch:t}catch(f){return E.error("DB","Error getting boundary timestamps",void 0,{error:f,project:o}),{observations:[],sessions:[],prompts:[]}}}let l=`
      SELECT *
      FROM observations
    ...
    `).run(e.content_session_id,e.prompt_number,e.prompt_text,e.created_at,e.created_at_epoch).lastInsertRowid}}storeAIAnalysis(e,t,s,n=[],o=[],i=[],a=0){let d=Date.now(),c=new Date(d).toISOString();return this.db.transaction(()=>{let T=this.db.prepare(`
        INSERT INTO ai_analysis
        (memory_session_id, project, analysis_text, key_insights, connections,
         discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(e,t,s,n.length>0?JSON.stringify(n):null,o.length>0?JSON.stringify(o):null,a,c,d),m=Number(T.lastInsertRowid);if(i.length>0){let u=this.db.prepare(`
          UPDATE observations
          SET ai_analysis_id = ?
          WHERE id = ?
        `);for(let b of i)u.run(m,b);E.info("DB","Linked observations to AI analysis",{analysisId:m,observationCount:i.length,project:t})}return{id:m,createdAtEpoch:d}})()}getAIAnalysisById(e){let s=this.db.prepare(`
...
`):null}function xt(r,e,t,s,n,o){let i=[];o?i.push(...Ke(r)):i.push(...xe(r));let a=null,d="",c=!1;for(let l of e)if(l.type==="summary"){c&&(i.push(""),c=!1,a=null,d="");let p=l.data,T=nt(p.displayTime);o?i.push(...Qe(p,T)):i.push(...$e(p,T))}else{let p=l.data,T=at(p.files_modified,n,p.files_read),m=ot(p.created_at),u=m!==d,b=u?m:"";d=m;let S=t.has(p.id);if(T!==a&&(c&&i.push(""),o?i.push(...qe(T)):i.push(...ke(T)),a=T,c=!0),S){let g=Ut(p,s);o?i.push(...ze(p,m,u,g,s)):(c&&!o&&(i.push(""),c=!1),i.push(...Fe(p,b,g,s)),a=null)}else o?i.push(Je(p,m,u,s)):i.push(we(p,b,s))}return c&&i.push(""),i}function dt(r,e,t,s,n){let o=[],i=vt(r);for(let[a,d]of i)o.push(...xt(a,d,e,t,s,n));return o}function _t(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function ct(r,e){let t=[];return e?(t.push(...x("Investigated",r.investigated,_.blue)),t.push(...x("Learned",r.learned,_.yellow)),t.push(...x("Completed",r.completed,_.green)),t.push(...x("Next Steps",r.next_steps,_.magenta))):(t.push(...U("Investigated",r.investigated)),t.push(...U("Learned",r.learned)),t.push(...U("Completed",r.completed)),t.push(...U("Next Steps",r.next_steps))),t}function Et(r,e){return e?Ze(r):Pe(r)}function lt(r,e,t){return!P(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?et(r.totalDiscoveryTokens,r.totalReadTokens):Xe(r.totalDiscoveryTokens,r.totalReadTokens)}var kt=pt.default.join((0,mt.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function wt(){try{return new F}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,ut.unlinkSync)(kt)}catch(e){E.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return E.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function Ft(r,e){return e?tt(r):Ge(r)}function $t(r,e,t,s,n,o,i){let a=[],d=q(e);a.push(...st(r,d,s,i));let c=t.slice(0,s.sessionCount),l=Ae(c,t),p=Z(e,l),T=Ce(e,s.fullObservationCount);a.push(...dt(p,T,s,n,i));let m=t[0],u=e[0];_t(s,m,u)&&a.push(...ct(m,i));let b=Q(e,s,o,n);return a.push(...Et(b,i)),a.push(...lt(d,s,i)),a.join(`
`).trimEnd()}async function te(r,e=!1,t){let s=Y(t),n=r?.cwd??process.cwd(),o=fe(n),i=r?.projects||[o],a=wt();if(!a)return"";try{let d=i.length>1?Re(a,i,s):J(a,o,s),c=i.length>1?Ie(a,i,s):z(a,o,s);return d.length===0&&c.length===0?Ft(o,e):$t(o,d,c,s,n,r?.session_id,e)}finally{a.close()}}0&&(module.exports={generateContext});
