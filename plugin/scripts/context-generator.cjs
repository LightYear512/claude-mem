...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),p=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",l={...s,location:p};return this.warn(e,`[HAPPY-PATH] ${t}`,l,n),o}},E=new W;var Ct={};function It(){return typeof __dirname<"u"?__dirname:(0,h.dirname)((0,ce.fileURLToPath)(Ct.url))}var At=It(),R=C.get("CLAUDE_MEM_DATA_DIR"),y=process.env.CLAUDE_CONFIG_DIR||(0,h.join)((0,de.homedir)(),".claude"),qt=(0,h.join)(y,"plugins","marketplaces","thedotmack"),Jt=(0,h.join)(R,"archives"),Qt=(0,h.join)(R,"logs"),zt=(0,h.join)(R,"trash"),Zt=(0,h.join)(R,"backups"),es=(0,h.join)(R,"modes"),ts=(0,h.join)(R,"settings.json"),Ee=(0,h.join)(R,"claude-mem.db"),ss=(0,h.join)(R,"vector-db"),rs=(0,h.join)(R,"observer-sessions"),ns=(0,h.join)(y,"settings.json"),os=(0,h.join)(y,"commands"),is=(0,h.join)(y,"CLAUDE.md");function pe(r){(0,_e.mkdirSync)(r,{recursive:!0})}function le(){return(0,h.join)(At,"..")}var me=require("crypto");var yt=3e4;function ue(r,e,t){return(0,me.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function Te(r,e,t){let s=t-yt;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var F=class{db;constructor(e=Ee){e!==":memory:"&&pe(R),this.db=new ge.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createAIAnalysisTable(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.createBudgetTables()}initializeSchema(){this.db.run(`
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),c=ue(e,s.title,s.narrative),p=Te(this.db,c,a);if(p)return{id:p.id,createdAtEpoch:p.created_at_epoch};let T=this.db.prepare(`
      INSERT INTO observations
      (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,c,d,a);return{id:Number(T.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),p=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(p.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),c=new Date(d).toISOString();return this.db.transaction(()=>{let l=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of s){let f=T.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),o||null,i,c,d);l.push(Number(f.lastInsertRowid))}let m=null;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,c,d);m=Number(f.lastInsertRowid)}return{observationIds:l,summaryId:m,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,c){let p=c??Date.now(),l=new Date(p).toISOString();return this.db.transaction(()=>{let m=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ...
      `).run(p,o),{observationIds:m,summaryId:f,createdAtEpoch:p}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),c=[...e],p=o?`WHERE id IN (${d}) AND project = ?`:`WHERE id IN (${d})`;return o&&c.push(o),this.db.prepare(`
      SELECT * FROM session_summaries
      ${p}
      ORDER BY created_at_epoch ${i}
      ${a}
    `).all(...c)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),c=[...e],p=o?"AND s.project = ?":"";return o&&c.push(o),this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.memory_session_id
    ...
    `).all(...c)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,c;if(e!==null){let u=`
      ...
      `,S=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let b=this.db.prepare(u).all(t,...a,s),O=this.db.prepare(S).all(t,...a,n+1);if(b.length===0&&O.length===0)return{observations:[],sessions:[],prompts:[]};d=b.length>0?b[b.length-1].created_at_epoch:t,c=O.length>0?O[O.length-1].created_at_epoch:t}catch(b){return E.error("DB","Error getting boundary timestamps",void 0,{error:b,project:o}),{observations:[],sessions:[],prompts:[]}}}let p=`
      SELECT *
      FROM observations
    ...
    `).run(e.content_session_id,e.prompt_number,e.prompt_text,e.created_at,e.created_at_epoch).lastInsertRowid}}storeAIAnalysis(e,t,s,n=[],o=[],i=[],a=0){let d=Date.now(),c=new Date(d).toISOString();return this.db.transaction(()=>{let T=this.db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(e,t,s,n.length>0?JSON.stringify(n):null,o.length>0?JSON.stringify(o):null,a,c,d),m=Number(T.lastInsertRowid);if(i.length>0){let g=this.db.prepare(`
          UPDATE observations
          SET ai_analysis_id = ?
          WHERE id = ?
        `);for(let f of i)g.run(m,f);E.info("DB","Linked observations to AI analysis",{analysisId:m,observationCount:i.length,project:t})}return{id:m,createdAtEpoch:d}})()}getAIAnalysisById(e){let s=this.db.prepare(`
