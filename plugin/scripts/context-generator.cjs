...
`+JSON.stringify(o,null,2):l=" "+this.formatData(o));let p="";if(n){let{sessionId:b,memorySessionId:f,correlationId:m,...g}=n;Object.keys(g).length>0&&(p=` {${Object.entries(g).map(([O,Tt])=>`${O}=${Tt}`).join(", ")}}`)}let T=`[${i}] [${a}] [${d}] ${c}${s}${p}${l}`,u=Rt(T);if(this.logFilePath)try{(0,A.appendFileSync)(this.logFilePath,u+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),l=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",p={...s,location:l};return this.warn(e,`[HAPPY-PATH] ${t}`,p,n),o}},E=new W;var Ct={};function It(){return typeof __dirname<"u"?__dirname:(0,h.dirname)((0,ce.fileURLToPath)(Ct.url))}var At=It(),R=C.get("CLAUDE_MEM_DATA_DIR"),L=process.env.CLAUDE_CONFIG_DIR||(0,h.join)((0,de.homedir)(),".claude"),qt=(0,h.join)(L,"plugins","marketplaces","thedotmack"),Jt=(0,h.join)(R,"archives"),Qt=(0,h.join)(R,"logs"),zt=(0,h.join)(R,"trash"),Zt=(0,h.join)(R,"backups"),es=(0,h.join)(R,"modes"),ts=(0,h.join)(R,"settings.json"),Ee=(0,h.join)(R,"claude-mem.db"),ss=(0,h.join)(R,"vector-db"),rs=(0,h.join)(R,"observer-sessions"),ns=(0,h.join)(L,"settings.json"),os=(0,h.join)(L,"commands"),is=(0,h.join)(L,"CLAUDE.md");function le(r){(0,_e.mkdirSync)(r,{recursive:!0})}function pe(){return(0,h.join)(At,"..")}var me=require("crypto");var Lt=3e4;function ue(r,e,t){return(0,me.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function Te(r,e,t){let s=t-Lt;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var F=class{db;constructor(e=Ee){e!==":memory:"&&le(R),this.db=new ge.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createAIAnalysisTable(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.createBudgetTables(),this.addContentSessionIdToObservations()}initializeSchema(){this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:d}=t,c=s==="date_asc"?"ASC":"DESC",l=n?`LIMIT ${n}`:"",p=e.map(()=>"?").join(","),T=[...e],u=[];if(o&&(u.push("project = ?"),T.push(o)),i)if(Array.isArray(i)){let m=i.map(()=>"?").join(",");u.push(`type IN (${m})`),T.push(...i)}else u.push("type = ?"),T.push(i);if(a){let m=Array.isArray(a)?a:[a],g=m.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");T.push(...m),u.push(`(${g.join(" OR ")})`)}if(d){let m=Array.isArray(d)?d:[d],g=m.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");m.forEach(S=>{T.push(`%${S}%`,`%${S}%`)}),u.push(`(${g.join(" OR ")})`)}let b=u.length>0?`WHERE id IN (${p}) AND ${u.join(" AND ")}`:`WHERE id IN (${p})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${b}
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i,a){let d=i??Date.now(),c=new Date(d).toISOString(),l=ue(e,s.title,s.narrative),p=Te(this.db,l,d);if(p)return{id:p.id,createdAtEpoch:p.created_at_epoch};let T=a||null;T||(T=this.db.prepare("SELECT content_session_id FROM sdk_sessions WHERE memory_session_id = ?").get(e)?.content_session_id||null);let b=this.db.prepare(`
      INSERT INTO observations
      (memory_session_id, content_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,T,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,l,c,d);return{id:Number(b.lastInsertRowid),createdAtEpoch:d}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),l=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(l.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a,d){let c=a??Date.now(),l=new Date(c).toISOString(),p=d||null;return p||(p=this.db.prepare("SELECT content_session_id FROM sdk_sessions WHERE memory_session_id = ?").get(e)?.content_session_id||null),this.db.transaction(()=>{let u=[],b=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, content_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let m of s){let g=b.run(e,p,t,m.type,m.title,m.subtitle,JSON.stringify(m.facts),m.narrative,JSON.stringify(m.concepts),JSON.stringify(m.files_read),JSON.stringify(m.files_modified),o||null,i,l,c);u.push(Number(g.lastInsertRowid))}let f=null;if(n){let g=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,l,c);f=Number(g.lastInsertRowid)}return{observationIds:u,summaryId:f,createdAtEpoch:c}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,c){let l=c??Date.now(),p=new Date(l).toISOString();return this.db.transaction(()=>{let u=[],b=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of s){let S=b.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),a||null,d,p,l);u.push(Number(S.lastInsertRowid))}let f;if(n){let S=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,p,l);f=Number(S.lastInsertRowid)}return this.db.prepare(`
      ...
      `).run(l,o),{observationIds:u,summaryId:f,createdAtEpoch:l}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),c=[...e],l=o?`WHERE id IN (${d}) AND project = ?`:`WHERE id IN (${d})`;return o&&c.push(o),this.db.prepare(`
      SELECT * FROM session_summaries
      ${l}
      ORDER BY created_at_epoch ${i}
      ${a}
    `).all(...c)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),c=[...e],l=o?"AND s.project = ?":"";return o&&c.push(o),this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.memory_session_id
    ...
    `).all(...c)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,c;if(e!==null){let m=`
      ...
      `,g=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let S=this.db.prepare(m).all(e,...a,s+1),O=this.db.prepare(g).all(e,...a,n+1);if(S.length===0&&O.length===0)return{observations:[],sessions:[],prompts:[]};d=S.length>0?S[S.length-1].created_at_epoch:t,c=O.length>0?O[O.length-1].created_at_epoch:t}catch(S){return E.error("DB","Error getting boundary observations",void 0,{error:S,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let m=`
      ...
      `,g=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let S=this.db.prepare(m).all(t,...a,s),O=this.db.prepare(g).all(t,...a,n+1);if(S.length===0&&O.length===0)return{observations:[],sessions:[],prompts:[]};d=S.length>0?S[S.length-1].created_at_epoch:t,c=O.length>0?O[O.length-1].created_at_epoch:t}catch(S){return E.error("DB","Error getting boundary timestamps",void 0,{error:S,project:o}),{observations:[],sessions:[],prompts:[]}}}let l=`
      SELECT *
      FROM observations
    ...
    `).run(e.content_session_id,e.prompt_number,e.prompt_text,e.created_at,e.created_at_epoch).lastInsertRowid}}storeAIAnalysis(e,t,s,n=[],o=[],i=[],a=0){let d=Date.now(),c=new Date(d).toISOString();return this.db.transaction(()=>{let T=this.db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(e,t,s,n.length>0?JSON.stringify(n):null,o.length>0?JSON.stringify(o):null,a,c,d),u=Number(T.lastInsertRowid);if(i.length>0){let b=this.db.prepare(`
          UPDATE observations
          SET ai_analysis_id = ?
          WHERE id = ?
        `);for(let f of i)b.run(u,f);E.info("DB","Linked observations to AI analysis",{analysisId:u,observationCount:i.length,project:t})}return{id:u,createdAtEpoch:d}})()}getAIAnalysisById(e){let s=this.db.prepare(`
