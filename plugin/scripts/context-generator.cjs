...
`+JSON.stringify(o,null,2):p=" "+this.formatData(o));let l="";if(n){let{sessionId:g,memorySessionId:f,correlationId:T,...S}=n;Object.keys(S).length>0&&(l=` {${Object.entries(S).map(([O,Tt])=>`${O}=${Tt}`).join(", ")}}`)}let m=`[${i}] [${a}] [${d}] ${E}${s}${l}${p}`,u=Rt(m);if(this.logFilePath)try{(0,A.appendFileSync)(this.logFilePath,u+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),p=E?`${E[1].split("/").pop()}:${E[2]}`:"unknown",l={...s,location:p};return this.warn(e,`[HAPPY-PATH] ${t}`,l,n),o}},c=new W;var Ct={};function It(){return typeof __dirname<"u"?__dirname:(0,h.dirname)((0,ce.fileURLToPath)(Ct.url))}var At=It(),R=C.get("CLAUDE_MEM_DATA_DIR"),y=process.env.CLAUDE_CONFIG_DIR||(0,h.join)((0,de.homedir)(),".claude"),qt=(0,h.join)(y,"plugins","marketplaces","thedotmack"),Jt=(0,h.join)(R,"archives"),Qt=(0,h.join)(R,"logs"),zt=(0,h.join)(R,"trash"),Zt=(0,h.join)(R,"backups"),es=(0,h.join)(R,"modes"),ts=(0,h.join)(R,"settings.json"),Ee=(0,h.join)(R,"claude-mem.db"),ss=(0,h.join)(R,"vector-db"),rs=(0,h.join)(R,"observer-sessions"),ns=(0,h.join)(y,"settings.json"),os=(0,h.join)(y,"commands"),is=(0,h.join)(y,"CLAUDE.md");function pe(r){(0,_e.mkdirSync)(r,{recursive:!0})}function le(){return(0,h.join)(At,"..")}var me=require("crypto");var yt=3e4;function ue(r,e,t){return(0,me.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function Te(r,e,t){let s=t-yt;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var F=class{db;constructor(e=Ee){e!==":memory:"&&pe(R),this.db=new ge.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createAIAnalysisTable(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.createBudgetTables()}initializeSchema(){this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:d}=t,E=s==="date_asc"?"ASC":"DESC",p=n?`LIMIT ${n}`:"",l=e.map(()=>"?").join(","),m=[...e],u=[];if(o&&(u.push("project = ?"),m.push(o)),i)if(Array.isArray(i)){let T=i.map(()=>"?").join(",");u.push(`type IN (${T})`),m.push(...i)}else u.push("type = ?"),m.push(i);if(a){let T=Array.isArray(a)?a:[a],S=T.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");m.push(...T),u.push(`(${S.join(" OR ")})`)}if(d){let T=Array.isArray(d)?d:[d],S=T.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");T.forEach(b=>{m.push(`%${b}%`,`%${b}%`)}),u.push(`(${S.join(" OR ")})`)}let g=u.length>0?`WHERE id IN (${l}) AND ${u.join(" AND ")}`:`WHERE id IN (${l})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${g}
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),E=ue(e,s.title,s.narrative),p=Te(this.db,E,a);if(p)return{id:p.id,createdAtEpoch:p.created_at_epoch};let m=this.db.prepare(`
      INSERT INTO observations
      (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,E,d,a);return{id:Number(m.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),p=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(p.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),E=new Date(d).toISOString();return this.db.transaction(()=>{let l=[],m=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of s){let f=m.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),o||null,i,E,d);l.push(Number(f.lastInsertRowid))}let u=null;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,E,d);u=Number(f.lastInsertRowid)}return{observationIds:l,summaryId:u,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,E){let p=E??Date.now(),l=new Date(p).toISOString();return this.db.transaction(()=>{let u=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let S of s){let b=g.run(e,t,S.type,S.title,S.subtitle,JSON.stringify(S.facts),S.narrative,JSON.stringify(S.concepts),JSON.stringify(S.files_read),JSON.stringify(S.files_modified),a||null,d,l,p);u.push(Number(b.lastInsertRowid))}let f;if(n){let b=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ...
      `).run(p,o),{observationIds:u,summaryId:f,createdAtEpoch:p}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),E=[...e],p=o?`WHERE id IN (${d}) AND project = ?`:`WHERE id IN (${d})`;return o&&E.push(o),this.db.prepare(`
    ...
    `).all(...E)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),E=[...e],p=o?"AND s.project = ?":"";return o&&E.push(o),this.db.prepare(`
      ...
      ${a}
    `).all(...E)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,E;if(e!==null){let T=`
    ...
    `).run(e.content_session_id,e.prompt_number,e.prompt_text,e.created_at,e.created_at_epoch).lastInsertRowid}}storeAIAnalysis(e,t,s,n=[],o=[],i=[],a=0){let d=Date.now(),E=new Date(d).toISOString(),l=this.db.prepare(`
      ...
      (memory_session_id, project, analysis_text, key_insights, connections,
       discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s,n.length>0?JSON.stringify(n):null,o.length>0?JSON.stringify(o):null,a,E,d),m=Number(l.lastInsertRowid);if(i.length>0){let u=this.db.prepare(`
        UPDATE observations
        SET ai_analysis_id = ?
        WHERE id = ?
      `);for(let g of i)u.run(m,g);c.info("DB","Linked observations to AI analysis",{analysisId:m,observationCount:i.length,project:t})}return{id:m,createdAtEpoch:d}}getAIAnalysisById(e){let s=this.db.prepare(`
