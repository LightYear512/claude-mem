...
`+JSON.stringify(o,null,2):l=" "+this.formatData(o));let p="";if(n){let{sessionId:g,memorySessionId:h,correlationId:m,...S}=n;Object.keys(S).length>0&&(p=` {${Object.entries(S).map(([N,Tt])=>`${N}=${Tt}`).join(", ")}}`)}let T=`[${i}] [${a}] [${d}] ${c}${s}${p}${l}`,u=Rt(T);if(this.logFilePath)try{(0,A.appendFileSync)(this.logFilePath,u+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),l=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",p={...s,location:l};return this.warn(e,`[HAPPY-PATH] ${t}`,p,n),o}},E=new W;var Ct={};function It(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,ce.fileURLToPath)(Ct.url))}var At=It(),R=C.get("CLAUDE_MEM_DATA_DIR"),y=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,de.homedir)(),".claude"),qt=(0,b.join)(y,"plugins","marketplaces","thedotmack"),Jt=(0,b.join)(R,"archives"),zt=(0,b.join)(R,"logs"),Qt=(0,b.join)(R,"trash"),Zt=(0,b.join)(R,"backups"),es=(0,b.join)(R,"modes"),ts=(0,b.join)(R,"settings.json"),Ee=(0,b.join)(R,"claude-mem.db"),ss=(0,b.join)(R,"vector-db"),rs=(0,b.join)(R,"observer-sessions"),ns=(0,b.join)(y,"settings.json"),os=(0,b.join)(y,"commands"),is=(0,b.join)(y,"CLAUDE.md");function le(r){(0,_e.mkdirSync)(r,{recursive:!0})}function pe(){return(0,b.join)(At,"..")}var ue=require("crypto");var yt=3e4;function me(r,e,t){return(0,ue.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function Te(r,e,t){let s=t-yt;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var F=class{db;constructor(e=Ee){e!==":memory:"&&le(R),this.db=new ge.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createAIAnalysisTable(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.createBudgetTables()}initializeSchema(){this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:d}=t,c=s==="date_asc"?"ASC":"DESC",l=n?`LIMIT ${n}`:"",p=e.map(()=>"?").join(","),T=[...e],u=[];if(o&&(u.push("project = ?"),T.push(o)),i)if(Array.isArray(i)){let m=i.map(()=>"?").join(",");u.push(`type IN (${m})`),T.push(...i)}else u.push("type = ?"),T.push(i);if(a){let m=Array.isArray(a)?a:[a],S=m.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");T.push(...m),u.push(`(${S.join(" OR ")})`)}if(d){let m=Array.isArray(d)?d:[d],S=m.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");m.forEach(f=>{T.push(`%${f}%`,`%${f}%`)}),u.push(`(${S.join(" OR ")})`)}let g=u.length>0?`WHERE id IN (${p}) AND ${u.join(" AND ")}`:`WHERE id IN (${p})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${g}
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),c=me(e,s.title,s.narrative),l=Te(this.db,c,a);if(l)return{id:l.id,createdAtEpoch:l.created_at_epoch};let T=this.db.prepare(`
    ...
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(l.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),c=new Date(d).toISOString();return this.db.transaction(()=>{let p=[],T=this.db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of s){let h=T.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),o||null,i,c,d);p.push(Number(h.lastInsertRowid))}let u=null;if(n){let h=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,c,d);u=Number(h.lastInsertRowid)}return{observationIds:p,summaryId:u,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,c){let l=c??Date.now(),p=new Date(l).toISOString();return this.db.transaction(()=>{let u=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let S of s){let f=g.run(e,t,S.type,S.title,S.subtitle,JSON.stringify(S.facts),S.narrative,JSON.stringify(S.concepts),JSON.stringify(S.files_read),JSON.stringify(S.files_modified),a||null,d,p,l);u.push(Number(f.lastInsertRowid))}let h;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,p,l);h=Number(f.lastInsertRowid)}return this.db.prepare(`
      ...
      `).run(l,o),{observationIds:u,summaryId:h,createdAtEpoch:l}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),c=[...e],l=o?`WHERE id IN (${d}) AND project = ?`:`WHERE id IN (${d})`;return o&&c.push(o),this.db.prepare(`
    ...
    `).all(...c)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,c;if(e!==null){let m=`
      ...
      `,S=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let f=this.db.prepare(m).all(e,...a,s+1),N=this.db.prepare(S).all(e,...a,n+1);if(f.length===0&&N.length===0)return{observations:[],sessions:[],prompts:[]};d=f.length>0?f[f.length-1].created_at_epoch:t,c=N.length>0?N[N.length-1].created_at_epoch:t}catch(f){return E.error("DB","Error getting boundary observations",void 0,{error:f,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let m=`
      ...
      `,S=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let f=this.db.prepare(m).all(t,...a,s),N=this.db.prepare(S).all(t,...a,n+1);if(f.length===0&&N.length===0)return{observations:[],sessions:[],prompts:[]};d=f.length>0?f[f.length-1].created_at_epoch:t,c=N.length>0?N[N.length-1].created_at_epoch:t}catch(f){return E.error("DB","Error getting boundary timestamps",void 0,{error:f,project:o}),{observations:[],sessions:[],prompts:[]}}}let l=`
      SELECT *
      FROM observations
    ...
    `).run(e.content_session_id,e.prompt_number,e.prompt_text,e.created_at,e.created_at_epoch).lastInsertRowid}}storeAIAnalysis(e,t,s,n=[],o=[],i=[],a=0){let d=Date.now(),c=new Date(d).toISOString();return this.db.transaction(()=>{let T=this.db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(e,t,s,n.length>0?JSON.stringify(n):null,o.length>0?JSON.stringify(o):null,a,c,d),u=Number(T.lastInsertRowid);if(i.length>0){let g=this.db.prepare(`
          UPDATE observations
          SET ai_analysis_id = ?
          WHERE id = ?
        `);for(let h of i)g.run(u,h);E.info("DB","Linked observations to AI analysis",{analysisId:u,observationCount:i.length,project:t})}return{id:u,createdAtEpoch:d}})()}getAIAnalysisById(e){let s=this.db.prepare(`
...
`):null}function xt(r,e,t,s,n,o){let i=[];o?i.push(...Ke(r)):i.push(...xe(r));let a=null,d="",c=!1;for(let l of e)if(l.type==="summary"){c&&(i.push(""),c=!1,a=null,d="");let p=l.data,T=nt(p.displayTime);o?i.push(...Qe(p,T)):i.push(...$e(p,T))}else{let p=l.data,T=at(p.files_modified,n,p.files_read),u=ot(p.created_at),g=u!==d,h=g?u:"";d=u;let m=t.has(p.id);if(T!==a&&(c&&i.push(""),o?i.push(...qe(T)):i.push(...ke(T)),a=T,c=!0),m){let S=Ut(p,s);o?i.push(...ze(p,u,g,S,s)):(c&&!o&&(i.push(""),c=!1),i.push(...Fe(p,h,S,s)),a=null)}else o?i.push(Je(p,u,g,s)):i.push(we(p,h,s))}return c&&i.push(""),i}function dt(r,e,t,s,n){let o=[],i=vt(r);for(let[a,d]of i)o.push(...xt(a,d,e,t,s,n));return o}function _t(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function ct(r,e){let t=[];return e?(t.push(...x("Investigated",r.investigated,_.blue)),t.push(...x("Learned",r.learned,_.yellow)),t.push(...x("Completed",r.completed,_.green)),t.push(...x("Next Steps",r.next_steps,_.magenta))):(t.push(...U("Investigated",r.investigated)),t.push(...U("Learned",r.learned)),t.push(...U("Completed",r.completed)),t.push(...U("Next Steps",r.next_steps))),t}function Et(r,e){return e?Ze(r):Pe(r)}function lt(r,e,t){return!P(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?et(r.totalDiscoveryTokens,r.totalReadTokens):Xe(r.totalDiscoveryTokens,r.totalReadTokens)}var kt=pt.default.join((0,ut.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function wt(){try{return new F}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,mt.unlinkSync)(kt)}catch(e){E.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return E.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function Ft(r,e){return e?tt(r):Ge(r)}function $t(r,e,t,s,n,o,i){let a=[],d=q(e);a.push(...st(r,d,s,i));let c=t.slice(0,s.sessionCount),l=Ae(c,t),p=Z(e,l),T=Ce(e,s.fullObservationCount);a.push(...dt(p,T,s,n,i));let u=t[0],g=e[0];_t(s,u,g)&&a.push(...ct(u,i));let h=Q(e,s,o,n);return a.push(...Et(h,i)),a.push(...lt(d,s,i)),a.join(`
